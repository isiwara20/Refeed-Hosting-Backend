...
export const createComplaintController = async (req, res) => {
  try {
    ...
    await notificationService.triggerComplaintCreatedNotification({
      userId: complaint.reportedBy, // Use reportedBy which is the username
      complaintId: complaint._id?.toString(),
      category: complaint.category,
      subject: complaint.title || complaint.description?.substring(0, 50)
...
export const updateComplaintController = async (req, res) => {
  try {
    ...
    await notificationService.triggerComplaintStatusUpdatedNotification({
      userId: updated.reportedBy, // Use reportedBy which is the username
    ...
    await notificationService.notifyAllAdminsInApp({
      ...
      message: `Complaint ${updated._id?.toString() || "(unknown)"} moved to ${req.body.status || updated.status || "updated"} by ${req.admin?.username || "admin"}.`,
      metadata: {
        complaintId: updated._id?.toString() || "",
        newStatus: req.body.status || updated.status || "",
        previousStatus: updated.previousStatus || "",
        updatedBy: req.admin?.username || "admin"
      }
    }).catch((err) => console.error("Admin broadcast for complaint status update failed:", err));
